...
export class TextWidget extends Widget<Phaser.GameObjects.Text, ITextWidgetProps> {
  ...
  create(scene: GameScene): void {
    ...
    if (style && style.fontSize) {
      this.gameObject.setFontSize(style.fontSize);
    }
    if (style && style.fontFamily) {
      this.gameObject.setFontFamily(style.fontFamily);
    }
    if (style && style.color) {
      this.gameObject.setColor(style.color);
    }
    if (style && style.stroke) {
      this.gameObject.setStroke(style.stroke, style.strokeThickness || 0);
    }
    if (style && style.fontStyle) {
      this.gameObject.setFontStyle(style.fontStyle);
    }
  }
